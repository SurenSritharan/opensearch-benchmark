...
class BulkVectorsFromDataSetParamSource(VectorDataSetPartitionParamSource):
    ...
    def partition(self, partition_index, total_partitions):
        partition = super().partition(partition_index, total_partitions)
        # Only the last partition emits last-batch=True so the runner only
        # issues the post-ingest refresh once across all parallel clients.
        partition.is_last_partition = self._is_last_partition(partition_index, total_partitions)
    ...
    def params(self):
        ...
        if is_last_batch and getattr(self, "is_last_partition", True):
